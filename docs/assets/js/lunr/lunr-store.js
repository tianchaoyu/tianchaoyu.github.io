var store = [{
        ...
        "title": "minimal-mistakes 对数学公式的支持",
        "excerpt":"在 Minimal Mistakes 主题中支持数学公式，可以通过集成 MathJax MathJax支持 在 _config.yml 文件中添加如下配置 markdown: kramdown 从minimal mistakes主题中, 复制 _includes/head.html 文件到个人博客 _includes 文件夹下； 在 _includes/head.html 文件中添加如下配置 &lt;!--数学公式支持--&gt; &lt;script type=\"text/javascript\" async src=\"https://cdnjs.cloudflare.com/ajax/libs/mathjax/2.7.7/MathJax.js?config=TeX-MML-AM_CHTML\"&gt; &lt;/script&gt; &lt;script type=\"text/x-mathjax-config\"&gt; MathJax.Hub.Config({ tex2jax: { inlineMath: [['$','$'], ['\\$','\\$']], processEscapes: true }, \"HTML-CSS\": { linebreaks: { automatic: true } }, \"SVG\": { linebreaks: {...","categories": ["notebook"],
        "tags": [],
        "url": "/tools/MathJax/",
        "teaser": null
      },{
        ...
        "title": "时段划分",
        "excerpt":"时间分布选择是将全天各方式出行OD矩阵，通过时间分布系数转换为分时段出行OD矩阵的过程。时间分布选择主要用于确定分目的、分群体、分方式的交通需求特征。时间分布系数的计算方法一般有以下几种： 以不同时段运行状态下的交通量作为时间分布系数的计算依据； 根据居民出行调查数据中的出行量等，计算时间分布系数； 其它可能的时间分布系数计算方法。宏观模型将全天时间分为五个时段，即早高峰（AM，AM Peak）、午平峰（MD，MidDay）、晚高峰（PM，PM Peak）、晚平峰（EN，Evening）、夜晚（NT，Night）， 根据上述计算方法确定每一个时段的时间分布系数。具体如下： 时间分布选择： 时段 缩写 对应时段   AM 07:00-10:00   MD 10:00-16:00   PM 16:00-19:00   EN 19:00-22:00   NT 22:00-07:00 出行时段组合 5个时段，出发时段和返回时段的组合共有15种，其中，AM-AM、MD-MD、PM-PM、EN-EN、NT-NT为同一时段。 组合 AM MD PM EN NT AM * * * * * MD   * * * * PM     *...","categories": [],
        "tags": [],
        ...
        "title": "Matsim评分机制",
        "excerpt":"Good Plans and Bad Plans, Score and Utility Matsim依赖于协同进化算法，每个智能体在移动仿真中执行多个方案，并给每一个方案打分，通过维护多个方案来学习，通过修改方案来保持进化提高。整个进化过程包括下面三大部分： 移动仿真：智能体选择某个方案，通过在模型路网中进行仿真测试，也叫做网络加载 计划打分：根据每个方案在仿真测试中的结果进行打分 重新计划：主要包括如下步骤： 如果智能体的方案数量超过最大值（配置文件中定义），然后根据方案选择器（选择）删除计划，减少方案的数量，每个智能体维护的方案数量是固定的，有配置文件觉得 计划会被复制、修改，然后选择用于下一次迭代（选择集扩展、创新）。 所有其他个体人在各自的计划之间进行选择。需要注意的是，Replanning的三个步骤必须配合工作方法：计划删除步骤应删除“坏”计划，创新步骤应生成“好”计划，而选择通常应选择好计划。这里，“好”的意思是“能够在mobsim/scoring中获得高分”。幸运的是，由于其进化的概念，该方法是相当稳健的：创新步骤并不总是产生好的解决方案。但是如果一些解决方案是好的，并导致高分，也已经足够。 1、方案评价基本效用函数公式 基础方程为，方案的效用值 等于 活动效应 $ S(act,q) $,加上出行效用值 $ S(trav,mde(q))$. \\[S_{p l a n}=\\sum_{q=0}^{N-1} S_{a c t, q}+\\sum_{q=0}^{N-1} S_{t r a v, \\operatorname{mode}(q)}\\] $ S_{pln}$: 方案计划的总效用 $S_{a c t, q}$:所有活动的总效用 $S_{t r a v, mode}$:出行过程的总效用 $N$表示活动的总数...","categories": [],
        "tags": [],
    ...
    "title": "技术教程",
    "excerpt":"                                                                                                                     Python入门基础                                                    python环境配置，项目管理，版本控制等入门基础与系统开发常用功能总结。                                                   开始学习                                                                                                                                                    交通规划软件Emme使用                                                    本教材是对之前使用Emme4的技术总结，由【入门篇、进阶篇、高级篇和应用篇】四个章节组成。                                                   开始学习                                                                                                                                                    交通规划软件Transcad使用                                                    由【入门篇、进阶篇、高级篇和应用篇】四个章节组成，简述TransCAD应用。                                                   开始学习                                                                                                                                                    交通模型技术总结                                                    由【四阶段模型、出行链模型、活动链和智能体模型】四部分组成，交通模型理论基础。                                                   开始学习                                                                                                                                                    机器学习与人工智能                                                    由【机器学习、人工智能、图神经网络和大模型】四部分组成。                                                   开始学习                                        ","url": "http://localhost:4000/tutorials/"
  }]
